<?php
/**
 * Taxonomy Archive: prompt_tag
 *
 * URLs: /prompts/tags/{slug}/ (see linkawy_register_prompt_tag_rewrite_rules).
 *
 * @package Linkawy
 */
...
$current_term = is_tax('prompt_tag') ? get_queried_object() : null;
$current_slug = ''; // prompt_type: none selected on tag archive
...
$current_tag_slug = ($current_term && !is_wp_error($current_term)) ? $current_term->slug : '';
$term_subtitle = '';
if ($current_term && !is_wp_error($current_term)) {
    $term_subtitle = term_description($current_term->term_id, 'prompt_tag');
}
?>
...
<section class="service-hero resource-hero-saas prompt-hero-no-media prompts-resource-hero">
    ...
    <div class="container service-hero-inner">
        <div class="service-hero-content">
            ...
            <h1 class="service-hero-title"><?php echo $current_term && !is_wp_error($current_term) ? esc_html($current_term->name) : esc_html__('وسم البرومبت', 'linkawy'); ?></h1>
            <?php if ($current_term && !is_wp_error($current_term)) : ?>
                <span class="resource-type-badge resource-type-badge-capsule">
                    <span class="resource-type-badge-text"><?php esc_html_e('وسم', 'linkawy'); ?></span>
                </span>
            <?php endif; ?>
            <p class="service-hero-subtitle"><?php echo $term_subtitle !== '' ? wp_kses_post($term_subtitle) : esc_html(__('اكتشف البرومبتات الموسومة بهذا الوسم.', 'linkawy')); ?></p>
        ...
        <div id="prompts-grid-wrap" class="prompts-grid-wrap">
            ...
            <div id="prompts-grid" class="blog-grid">
                <?php if (have_posts()) : ?>
                    ...
                    ?>
                <?php else : ?>
                    <p class="no-posts"><?php _e('لا توجد برومبتات بهذا الوسم.', 'linkawy'); ?></p>
                <?php endif; ?>
            </div>
